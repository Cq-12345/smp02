...
| 闭环 workflow | PiEvo-faithful 与 observation ledger 已有初版 | `src/smp02/pievo_faithful.py`，`docs/real_experiment_feedback_loop.md` | 将 ledger observation 按权重并入 full-history posterior |
| RL/人工闭环/真实实验 | 第三轮完成接入 schema | `trail/experiments/observation_schema.yaml`，`trail/experiments/import_observations.py` | 用真实实验替换示例占位行，建立审核流程 |
