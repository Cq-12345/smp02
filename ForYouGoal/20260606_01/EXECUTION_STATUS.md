...
- `scripts/evaluate_replacement_proposals.py`：把 VAE replacement proposals 重建为完整配方，送入 VAE-WVCM-GPR predictor 和 Harness。
...
| 生成模型 | replacement proposals 已进入 predictor、Harness、ledger 和 PiEvo | `reports/replacement_proposal_evaluation.md`，`reports/replacement_pievo_feedback_smoke.md` | 建立 generation record schema，覆盖 LLM/RAG/prompt 生成 |
| 闭环 workflow | PiEvo-faithful 已接收 ledger 加权历史，并修复短程选择浪费问题 | `src/smp02/pievo_faithful.py`，`artifacts/pievo_faithful_ledger_smoke/` | 对多个目标温度运行带 ledger 的 PiEvo，并比较 posterior |
...
- 通过项中最佳 replacement 预测 Tg 为 194.93 C，距 195 C 目标 0.07 C；4 条在 1 C 内，10 条在 5 C 内。
