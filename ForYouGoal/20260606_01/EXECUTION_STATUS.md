...
| 生成模型 | replacement proposals 已进入 predictor、Harness、ledger 和 PiEvo；generation record schema 已覆盖 LLM/RAG/prompt smoke | `reports/replacement_proposal_evaluation.md`，`reports/replacement_pievo_feedback_smoke.md`，`reports/generation_record_schema_smoke.md` | 接入真实 LLM/RAG agent，所有生成先写 generation ledger |
