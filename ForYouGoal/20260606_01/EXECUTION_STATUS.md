...
- 新增 `scripts/build_active_observation_ledger.py`：把 result intake 后的 observation ledger 再过滤成 active high-authority evidence ledger，作为后续 PiEvo posterior、策略更新或人工闭环总结的高权重证据入口。
- 默认输入是 `artifacts/trail/human_review/validation_result_observation_ledger.csv`；默认只允许 `high_fidelity_simulation`、`real_dsc`、`literature`，并且必须 `ledger_pass=true`。`surrogate` 不会进入这一层。
- 生成产物包括 `active_high_authority_observation_ledger.csv`、`active_high_authority_observation_summary.json` 和 `reports/active_high_authority_observation_ledger.md`。
- 当前输入 ledger 0 条完成观测，因此 `active_rows=0`、`validation_result_active_rows=0`、`authority_weight_sum=0.0`。这表示尚无完成且获批的高保真/真实/文献观测，没有绕过质量门；不是候选生成失败。
- `trail/workflow/multi_agent_workflow.py` 已接入 `active_observation_*` 字段，workflow summary 现在能同时看到 validation result intake 结果和真正可进入 active evidence 的行数、来源分布、authority weight、mean target distance。
- 文档已更新 `docs/closed_loop_workflow.md`、`docs/real_experiment_feedback_loop.md`、`docs/smp_research_system_plan.md` 和 `docs/generation_strategy_and_harness.md`，明确 request template、observation ledger 和 active evidence ledger 三层边界。
- 新增 `tests/test_active_observation_ledger.py`，并扩展 `tests/test_workflow_summary.py`，覆盖 high-fidelity/real/literature 通过项进入 active ledger、surrogate 和未通过 ledger gate 的项被排除、workflow summary 读取 active evidence summary。

## 第四十二轮结论

- PiEvo 外部观测加载器新增 `external_observation_allowed_source_types` 和 `external_observation_require_active_evidence` 两个配置项；默认不启用，避免影响既有 surrogate smoke，但 active evidence 配置会显式二级过滤来源和 active 标记。
- 新增 `configs/pievo_faithful_active_evidence_bridge_smoke.yaml`：读取 `active_high_authority_observation_ledger.csv`，只允许 `high_fidelity_simulation`、`real_dsc`、`literature`，且要求 `active_evidence=true`。
- 新增 `scripts/run_active_evidence_pievo_bridge.py`：使用 PiEvo 自身的 `load_external_observations`、`train_principle_experts` 和 `update_posterior_full_history`，验证 active evidence 是否已经能进入 principle posterior。
- 生成产物包括 `active_evidence_pievo_external_observations_used.csv`、`active_evidence_principle_posterior.json`、`active_evidence_pievo_bridge_summary.json` 和 `reports/active_evidence_pievo_bridge.md`。
- 当前 active ledger 为空，因此 bridge 输出 `bridge_status=no_active_evidence_noop`、`external_accepted_rows=0`、`posterior_history_rows=0`、`active_evidence_updates_posterior=false`。这表示 PiEvo posterior 没有新增高权重证据，是质量门正常生效。
- `trail/workflow/multi_agent_workflow.py` 已接入 `active_evidence_pievo_bridge_*` 字段，workflow summary 能看到 active evidence 是否实际更新了 PiEvo posterior、accepted rows、authority weight、posterior entropy 和 MAP principle。
- 新增 `tests/test_active_evidence_pievo_bridge.py`，并扩展 `tests/test_pievo_faithful_math.py`、`tests/test_workflow_summary.py`，覆盖 active/high-authority 过滤、surrogate/非 active 行拦截、bridge posterior 更新和 workflow summary 读取。

## 第四十三轮结论

- `scripts/update_generation_strategy_policy.py` 现在读取 `active_high_authority_observation_summary.json` 和 `active_evidence_pievo_bridge_summary.json`，把高权重 evidence 状态写入 strategy bandit summary/report。
- 新增 high-authority policy metadata：`high_authority_evidence_status`、`high_authority_budget_mode`、`high_authority_next_action`、`active_evidence_bridge_status`、`active_evidence_updates_pievo_posterior` 等字段。
- 当前重算结果为 `high_authority_evidence_status=awaiting_high_authority_evidence`、`high_authority_budget_mode=surrogate_backed_allocation`、`active_evidence_bridge_status=no_active_evidence_noop`。
- 这一步刻意不改变当前 allocation 数学打分：在 0 行高权重证据时，下一轮预算仍由 surrogate/generation evidence 计算，不能把缺失的真实/高保真结果当成奖励。
- `reports/generation_strategy_bandit_policy.md` 已新增 High-authority Evidence Gate 段，说明真实或高保真 evidence 进入 PiEvo posterior 后，应先比较 posterior shift，再调整 budget 权重。
- `trail/workflow/multi_agent_workflow.py` 已接入 `generation_strategy_policy_high_authority_*` 字段，workflow summary 可以同时看到 active evidence gate、PiEvo bridge 和 strategy policy 是否仍为 surrogate-backed。
- 扩展 `tests/test_generation_strategy_policy.py` 和 `tests/test_workflow_summary.py`，覆盖 high-authority context 的 waiting/active 两种状态，以及 run_policy 输出 summary/report。

## 第四十四轮结论

- `scripts/update_target_conditioned_generation_policy.py` 现在读取 `active_high_authority_observation_ledger.csv` 和 `active_evidence_pievo_bridge_summary.json`，把高权重 evidence 状态按 `target_tg_c` 汇总到目标条件化策略。
- 新增 target-wise high-authority metadata：`target_high_authority_evidence_status`、`target_high_authority_budget_mode`、`target_high_authority_rows_by_target`、`target_high_authority_authority_weight_by_target` 等字段。
- 当前重算结果为 `target_high_authority_evidence_status=awaiting_target_high_authority_evidence`、`target_high_authority_budget_mode=target_surrogate_backed_allocation`；190/195/200/250 C 的 active high-authority rows 全部为 0。
- 这一步同样不改变当前 target-conditioned allocation：每个目标 Tg 的预算仍由 target sweep 和 global-transfer surrogate evidence 计算，不能把 250 C surrogate pass 当成真实验证。
- `reports/target_conditioned_generation_strategy_policy.md` 已新增 Target High-authority Evidence Gate 段，并在 Target Summary 中显示每个目标的 active high-authority rows 和 authority weight。
- `trail/workflow/multi_agent_workflow.py` 已接入 `target_conditioned_strategy_policy_high_authority_*` 字段，workflow summary 可以看到全局策略和目标条件化策略都处于 high-authority no-op 状态。
- 扩展 `tests/test_target_conditioned_generation_policy.py` 和 `tests/test_workflow_summary.py`，覆盖 active high-authority rows by target、target high-authority summary 和 workflow summary 字段。

## 第四十五轮结论

- 新增 `scripts/build_validation_execution_schedule.py`：把 `validation_request_queue.csv` 转成 target-aware execution schedule 和 immediate execution batch。
- 该脚本只排程，不执行任务、不填写结果、不产生 observation；所有行都标记为 `execution_status=planned_not_completed`。
- 当前 55 个 validation requests 全部进入 schedule：30 个 `process_completion_now` 可立即执行，25 个 `blocked_until_process_completion` 仍等待对应工艺补全。
- 当前 immediate batch size 为 12；12 个 immediate tasks 全部是 `process_completion`，目标分布为 250 C 8 个、195 C 4 个，说明高 Tg sparse target 已进入实际执行优先队列。
- 25 个 blocked observation request 完成后最高 authority weight 为 3.0，但必须等 process completion 和 reviewer approval 后才能进入 result intake/active ledger。
- 生成产物包括 `validation_execution_schedule.csv`、`validation_execution_schedule_summary.json` 和 `reports/validation_execution_schedule.md`。
- `trail/workflow/multi_agent_workflow.py` 已接入 `validation_execution_*` 字段，workflow summary 能看到 schedule rows、immediate rows、blocked rows、process completion unlock rows、immediate batch target counts 和 phase counts。
- 新增 `tests/test_validation_execution_schedule.py`，并扩展 `tests/test_workflow_summary.py`，覆盖 blocked high-fidelity 不进入 immediate batch、process completion 解锁 observation request、workflow summary 读取 execution schedule。

## 第四十六轮结论

- 新增 `scripts/build_process_completion_packet.py`：把 validation execution schedule 的 immediate process completion batch 展开成可填写工艺字段包和 process record template。
- 当前 packet 有 12 行，全部匹配已有 draft process records；目标分布为 250 C 8 行、195 C 4 行。
- 输出包括 `process_completion_packet.csv`、`process_completion_process_record_template.csv`、`process_completion_process_record_ledger.csv`、`process_completion_packet_summary.json` 和 `reports/process_completion_packet.md`。
- 12 行基础 process record 格式全部通过，`process_record_pass_rows=12`；但 `ready_for_active_ledger_rows=0`、`process_incomplete_rows=12`，因为工艺字段尚未填写且未人工批准。
- 必填字段频次最高的是 `cure_temperature_c` 和 `post_cure_temperature_c`，各 10 次；`mix_temperature_c`、`cure_time_h`、`post_cure_time_h` 各 6 次。
- 这一步没有创建真实/高保真 observation，只是把“下一步该填什么工艺字段”落成 artifact，使 high-fidelity request 有明确解锁路径。
- `trail/workflow/multi_agent_workflow.py` 已接入 `process_completion_packet_*` 字段，workflow summary 能看到 packet rows、draft matches、unlock rows、process pass rows、ready rows 和 required field frequency。
- 新增 `tests/test_process_completion_packet.py`，并扩展 `tests/test_workflow_summary.py`，覆盖 packet 继承 draft process record、展开 required inputs、ready 仍为 0、workflow summary 读取 packet 状态。

## 第四十七轮结论

- 新增 `scripts/build_process_design_suggestion_packet.py`：把 process completion packet 继续转成知识模板驱动的工艺建议包，而不是让人工从空白字段开始补工艺。
- 输出包括 `process_design_suggestion_packet.csv`、`process_design_suggested_process_records.csv`、`process_design_suggested_process_record_ledger.csv`、`process_design_suggestion_summary.json` 和 `reports/process_design_suggestion_packet.md`。
- 当前 12 条建议覆盖 250 C 8 行、195 C 4 行；其中 8 行是高 Tg 工艺窗口，5 行 predictor sigma 不低于 75 C。
- 建议涉及 5 条 `epoxy_amine_thermal_cure`、4 条 `epoxy_anhydride_catalyzed_cure`、2 条 `anhydride_amine_imidization`、1 条 `maleimide_addition_or_copolymerization`。
- 脚本使用 process packet 必填字段与知识库 `process_condition_templates` 字段的并集生成建议，修复了 packet 字段和模板字段不完全一致时可能漏掉 `catalyst_loading` 的风险。
- 12 条 suggested process records 基础格式全部通过，12 条模板字段可补全；但所有行仍是 `knowledge_template_suggestion_not_observation`，`review_status=needs_human_review`，`ready_for_active_ledger_rows=0`。
- `trail/workflow/multi_agent_workflow.py` 已接入 `process_design_suggestion_*` 字段，workflow summary 能看到 suggestion rows、high Tg rows、high sigma rows、fields complete rows、ready rows、template counts 和 evidence level。
- 新增 `tests/test_process_design_suggestion_packet.py`，并扩展 `tests/test_workflow_summary.py`，覆盖模板字段并集、建议不升级 active ledger、workflow summary 读取 suggestion 状态。

## 第四十八轮结论

- 新增 `scripts/import_process_approval_intake.py`：把知识模板工艺建议转成人工审批入口，要求 `approval_decision`、`process_ready`、`reviewer_approved` 和 `reviewer_id` 同时满足。
- 输出包括 `process_completion_approval_template.csv`、`process_completion_approval_review.csv`、`process_completion_approved_process_records.csv`、`process_completion_approved_process_record_ledger.csv`、`process_completion_unblocked_validation_requests.csv`、`process_completion_approval_summary.json` 和 `reports/process_approval_intake.md`。
- 当前真实 artifacts 中有 12 行 approval template，但没有人工提交的 `process_completion_approval_completed.csv`，因此 `submitted_approval_rows=0`、`accepted_process_approval_rows=0`、`unblocked_observation_request_rows=0`。
- 审批通过后的 process record 会重新进入 `import_process_records`，必须 `ready_for_active_ledger=true`；测试覆盖了通过审批会解锁同一 `linked_observation_id` 的 high-fidelity request。
- 这一步仍不产生 Tg observation；即使 process approval 通过，也只是解锁后续 validation result intake。高保真/真实/文献结果仍必须单独填写 `observed_tg_c` 并通过 observation ledger 与 active evidence gate。
- `trail/workflow/multi_agent_workflow.py` 已接入 `process_approval_*` 字段，workflow summary 能看到 approval template rows、submitted rows、accepted rows、ready process record rows、unblocked request rows 和 gate status。
- 新增 `tests/test_process_approval_intake.py`，并扩展 `tests/test_workflow_summary.py`，覆盖人工审批门禁、拒绝原因、解锁请求和 workflow summary 读取。

## 第四十九轮结论

- 新增 `scripts/build_high_fidelity_protocol_packet.py`：把 `validation_request_queue.csv` 中的 25 条 `high_fidelity_validation` request 转成高保真/扩展集成模型的方法协议包，而不是直接生成 observation。
- 输出包括 `artifacts/trail/human_review/high_fidelity_protocol_packet.csv`、`artifacts/trail/human_review/high_fidelity_protocol_summary.json` 和 `reports/high_fidelity_protocol_packet.md`。
- 当前协议包共有 25 条：195 C 12 条、250 C 13 条；候选来源包括 `sparse_target_replacement_250=13`、`vae_latent_local_search=7`、`pievo_latent_local_search_selected=3`、`pievo_ensemble_guard_selected=1`、`expanded_inventory_replacement=1`。
- 当前 `ready_protocol_rows=0`、`blocked_protocol_rows=25`、`process_approval_unblocked_rows=0`，因为真实 artifacts 里仍没有人工 process approval 提交。所有协议仍是 `blocked_pending_process_approval`。
- 方法频次为 `process_feasibility_review=25`、`model_ensemble_recheck=25`、`high_fidelity_simulation_or_expanded_model_ensemble=25`、`thermal_stability_pre_screen=13`、`target_specific_literature_check=13`、`imidization_protocol_review=4`、`trimerization_catalyst_review=2`。
- 风险频次为 `process_incomplete=25`、`high_predictor_sigma=23`、`high_ood_penalty=22`、`new_component=14`、`high_tg_target=13`、`sparse_target_origin=13`。这说明高保真协议主要服务于高 Tg 稀疏目标和高不确定候选的复核。
- 协议包的 `evidence_level=high_fidelity_protocol_template_not_observation`，`creates_observation=false`；即使后续被 process approval 解锁，也只是允许进入 validation result intake，不会绕过 result intake、observation ledger 和 active evidence gate。
- `trail/workflow/multi_agent_workflow.py` 已接入 `high_fidelity_protocol_*` 字段，workflow summary 现在能看到协议行数、ready/blocked 行数、审批解锁行数、目标分布、方法频次、approval gate status 和 evidence level。
- 文档已更新 `docs/closed_loop_workflow.md`、`docs/real_experiment_feedback_loop.md`、`docs/smp_research_system_plan.md` 和 `docs/generation_strategy_and_harness.md`，把 high-fidelity protocol packet 放在 process approval 与 validation result intake 之间。
- 新增 `tests/test_high_fidelity_protocol_packet.py`，并扩展 `tests/test_workflow_summary.py`，覆盖空 unblocked CSV、不产生 observation、审批解锁后协议 ready 状态切换，以及 workflow summary 读取协议包状态。

## 第五十轮结论

- 新增 `scripts/build_validation_dependency_graph.py`：把 validation request、execution schedule、process approval、high-fidelity protocol、validation result intake 和 active evidence gate 形式化成 DAG 风格的节点/边表。
- 输出包括 `artifacts/trail/human_review/validation_dependency_nodes.csv`、`artifacts/trail/human_review/validation_dependency_edges.csv`、`artifacts/trail/human_review/validation_dependency_summary.json` 和 `reports/validation_dependency_graph.md`。
- 当前依赖图有 118 个节点、125 条边；由于没有人工 process approval 和完成高保真结果，125 条边全部处在 blocked/pending 状态。
- 当前 blocker reason 统计为 `process_approval_not_unblocked=50`、`human_process_approval_missing=25`、`protocol_not_ready=25`、`validation_result_missing_or_unapproved=25`。
- 当前 25 条 high-fidelity protocol 仍全部 blocked；目标分布为 195 C 12 条、250 C 13 条。active evidence rows 仍为 0。
- 依赖图识别的 critical path 下一步是 `review_process_completion_approval_template`，共有 12 行。这比单独看 packet 更直接地说明：下一步不是填 Tg 数字，而是先审 12 行工艺审批模板。
- 该图的 `evidence_level=validation_dependency_graph_not_observation`，只描述约束、依赖和下一步动作；不产生 Tg observation，也不更新 PiEvo posterior。
- `trail/workflow/multi_agent_workflow.py` 已接入 `validation_dependency_*` 字段，workflow summary 现在能看到节点数、边数、blocked/pending 边数、pending approval 行数、ready/blocked high-fidelity protocol 行数、active evidence 行数、下一步动作和 blocker counts。
- 文档已更新 `docs/closed_loop_workflow.md`、`docs/real_experiment_feedback_loop.md`、`docs/smp_research_system_plan.md` 和 `docs/generation_strategy_and_harness.md`，把依赖图放在 high-fidelity protocol 与 validation result intake 之间。
- 新增 `tests/test_validation_dependency_graph.py`，并扩展 `tests/test_workflow_summary.py`，覆盖 process approval 缺失时 protocol 被阻塞，以及审批通过后 high-fidelity protocol 进入可执行状态。

## 第五十一轮结论

- 新增 `scripts/build_predictor_model_registry.py`：把已训练 model zoo 的选择结果固化成可被 workflow 读取的模型注册表，而不是只停留在 `reports/model_selection_analysis.md` 文本里。
- 输出包括 `artifacts/trail/predictors/model_selection_registry/predictor_model_selection_registry.csv`、`artifacts/trail/predictors/model_selection_registry/predictor_model_selection_summary.json` 和 `reports/predictor_model_selection_registry.md`。
- 当前 registry 读取 343 行 metrics，其中 336 行是可用 joblib predictor，覆盖 latent size 16、32、64、128、256、512、1024。
- 默认闭环代理模型是 `VAE (512) + GaussianProcess_RBF`：MAPEK test 3.9778%、MAE test 18.7641 C、RMSE test 40.4371 C、R2 test 0.8233。
- 点误差备选是 `VAE (512) + NuSVR_RBF`：它在 MAE、RMSE 和 R2 上都优于 GPR；但它不能直接替代 GPR 的 uncertainty provider。
- uncertainty provider 仍是 `VAE (512) + GaussianProcess_RBF`，因此 PiEvo 的 sigma/IDS 路径保持可解释；候选排序仍同时保留 ensemble guard。
- ensemble guard members 固化为 6 个同 latent size 强模型：GPR、NuSVR、XGBoost depth3、ExtraTrees 300、ExtraTrees 600、GradientBoosting squared。
- `trail/workflow/multi_agent_workflow.py` 已接入 `predictor_model_registry_*` 字段，workflow summary 现在能看到 primary model、latent size、MAPEK/MAE/RMSE/R2、MAE/RMSE/R2 backup、uncertainty provider 和 ensemble member rows。
- 文档已更新 `docs/closed_loop_workflow.md`、`docs/smp_research_system_plan.md` 和 `docs/generation_strategy_and_harness.md`，明确后续默认使用哪个预测模型，以及为什么仍需要 ensemble disagreement。
- 新增 `tests/test_predictor_model_registry.py`，并扩展 `tests/test_workflow_summary.py`，覆盖 primary、backup、uncertainty provider 和 ensemble member 的选择规则。

## 第五十二轮结论

- 新增 `scripts/build_todo_completion_audit.py`：把 TODO 非暂缓任务的覆盖情况、证据文件和剩余边界转成可复算 artifact。
- 输出包括 `artifacts/trail/workflow/todo_completion_audit.csv`、`artifacts/trail/workflow/todo_completion_audit_summary.json` 和 `reports/todo_completion_audit.md`。
- 当前审计有 10 个条目：9 个 `implemented*`，1 个 `deferred_by_user`，即商品级/聚合物/超图表示按用户要求暂缓。
- 当前 `missing_evidence_rows=0`、`non_deferred_all_evidence_present=true`，说明非暂缓条目的代码/文档/artifact 证据都能在仓库中找到。
- 当前 `needs_real_or_high_fidelity_evidence_rows=2`，对应 generation models 和 human/real experiment loop；这不是脚本失败，而是后续需要真实外部 LLM/SFT/decoder 输出、人工 process approval、真实/高保真 observation。
- 当前主阻塞被审计为 `human_process_approval_and_real_or_high_fidelity_observation`。也就是说下一步不应伪造 Tg，而应先审核 12 行 process approval，再进入 high-fidelity/result intake/active evidence gate。
- `trail/workflow/multi_agent_workflow.py` 已接入 `todo_completion_*` 字段，workflow summary 现在能看到 TODO audit rows、implemented rows、deferred rows、needs real/high-fidelity evidence rows、missing evidence rows 和 primary blocker。
- 文档已更新 `docs/closed_loop_workflow.md` 与 `docs/smp_research_system_plan.md`，明确当前暂缓项和真实证据边界。
- 新增 `tests/test_todo_completion_audit.py`，并扩展 `tests/test_workflow_summary.py`，覆盖 evidence present/missing/deferred 计数和 workflow summary 读取。

## 第五十三轮结论

- 新增 `scripts/build_process_approval_reviewer_checklist.py`：把 12 行 `process_completion_approval_template.csv` 转成可执行人工审批审查清单，并连接到 downstream high-fidelity protocol。
- 输出包括 `artifacts/trail/human_review/process_approval_reviewer_checklist.csv`、`artifacts/trail/human_review/process_approval_reviewer_checklist_summary.json` 和 `reports/process_approval_reviewer_checklist.md`。
- 当前 checklist 有 12 行，全部 `ready_for_human_review=true`；`already_submitted_rows=0`、`accepted_process_approval_rows=0`，说明仍未有人类审批提交。
- 这 12 行可解锁 13 条 downstream high-fidelity protocol；目标分布为 195 C 4 行、250 C 8 行，候选来源以 `sparse_target_replacement_250=8` 为主。
- 建议字段频次最高的是 `cure_temperature_c=10`、`post_cure_temperature_c=10`、`mix_temperature_c=6`、`cure_time_h=6`、`post_cure_time_h=6`、`catalyst_loading=4`。
- 本轮修正了 `suggested_inputs` 字段值内部带分号时的解析问题：field frequency 现在只统计包含 `=` 的字段键，不把 `verify equivalent basis` 这类值说明误当字段。
- checklist 的 `evidence_level=process_approval_reviewer_checklist_not_observation`，`creates_observation=false`；它只是人工审查行动包，不产生 Tg，不解锁 active evidence。
- `trail/workflow/multi_agent_workflow.py` 已接入 `process_approval_reviewer_*` 字段，workflow summary 现在能看到 checklist rows、ready rows、submitted/accepted rows、可解锁 protocol rows、目标分布和建议字段频次。
- `scripts/build_todo_completion_audit.py` 已把 reviewer checklist 纳入 human/real experiment loop 的证据路径，TODO audit 仍保持 `missing_evidence_rows=0`。
- 文档已更新 `docs/closed_loop_workflow.md`、`docs/real_experiment_feedback_loop.md`、`docs/smp_research_system_plan.md` 和 `docs/generation_strategy_and_harness.md`。
- 新增 `tests/test_process_approval_reviewer_checklist.py`，并扩展 `tests/test_workflow_summary.py`，覆盖 downstream protocol 映射、accepted row 计数、非 observation 边界和 workflow summary 读取。

## 第五十四轮结论

- 新增 `scripts/build_external_generator_output_checklist.py`：把真实外部 LLM/SFT/SMILES decoder/diffusion-flow 输出接入前的门禁结构化，回应 TODO 中“LLM、SFT、RAG、扩散生成、流匹配”的真实输出边界。
- 输出包括 `artifacts/trail/generation/external_generator_output_checklist/external_generator_output_checklist.csv`、`external_generator_output_checklist_summary.json` 和 `reports/external_generator_output_checklist.md`。
- 当前 checklist 有 4 类 provider task：`llm_rag_principle_generation`、`sft_candidate_generator`、`diffusion_or_flow_matching` 三类可提交外部 generation records；`llm_smiles_generation` 仍 suppressed。
- 当前 `sft_ready=true`、`diffusion_flow_ready=true`，SFT examples 和 diffusion/flow seed rows 都为 268；这表示可以接真实外部输出，但输出必须先变成 generation record ledger。
- `llm_smiles_generation` 仍不能直接推荐，因为缺 predictor/chemistry evidence；free-form SMILES draft 只能作为 draft record 进入 RDKit、predictor、Harness 和人工审核。
- checklist 的 `creates_observation=false`、`evidence_level=external_generator_output_checklist_not_observation`；它不调用外部模型，也不会产生 Tg observation。
- `trail/workflow/multi_agent_workflow.py` 已接入 `external_generator_*` 字段，workflow summary 现在能看到 provider checklist rows、ready rows、suppressed rows、SFT/flow readiness、seed rows 和 ready/blocked strategy counts。
- `scripts/build_todo_completion_audit.py` 已把 `reports/external_generator_output_checklist.md` 纳入 generation model 证据路径；TODO audit 仍保持 `missing_evidence_rows=0`。
- 文档已更新 `docs/closed_loop_workflow.md`、`docs/generation_strategy_and_harness.md` 和 `docs/smp_research_system_plan.md`。
- 新增 `tests/test_external_generator_output_checklist.py`，并扩展 `tests/test_workflow_summary.py`，覆盖 ready/suppressed provider 状态、非 observation 边界和 workflow summary 读取。

## 第五十五轮结论

- 新增 `scripts/build_goal_completion_certificate.py`：把 `todo_completion_audit`、`multi_agent_summary`、人工审批门禁和外部生成门禁合并成 goal 退出前的严格完成证书。
- 输出包括 `artifacts/trail/workflow/goal_completion_certificate_requirements.csv`、`artifacts/trail/workflow/goal_completion_certificate_summary.json` 和 `reports/goal_completion_certificate.md`。
- 当前证书共有 10 条 requirement verdict：9 条非暂缓 TODO 任务均为 `implemented_and_evidence_present`，1 条 `commodity_polymer_hypergraph_representation` 按用户明确要求暂缓；`missing_evidence_rows=0`、`non_deferred_all_evidence_present=true`。
- 当前 `goal_exit_eligible=true`，其含义是非暂缓实现任务已有代码、artifact、测试和中文文档证据；它不是 Tg observation，也不代表真实 DSC 或高保真结果已经完成。
- 证书额外检查外部门禁：`process_approval_reviewer_ready_rows=12`、`external_generator_ready_provider_rows=3`、下一步仍是 `review_process_completion_approval_template`。这说明后续真正进入物理/高权重证据前，应先审 12 行工艺审批模板，再接入外部生成或高保真结果。
- 证书同时检查没有伪造高权重证据：`active_observation_rows=0`、`active_evidence_updates_pievo_posterior=false`。因此 PiEvo posterior 没有被 surrogate 或模板结果冒充真实 evidence 更新。
- 文档已更新 `docs/closed_loop_workflow.md` 和 `docs/smp_research_system_plan.md`，把证书作为 TODO 收尾审计层，而不是实验结果层。
- 新增 `tests/test_goal_completion_certificate.py`，覆盖正常可退出、缺 evidence 拒绝退出、暂缓范围不匹配拒绝退出。
