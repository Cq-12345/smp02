...
- `scripts/build_high_fidelity_protocol_packet.py` 已把 25 条 high-fidelity request 转成方法协议包；195 C 12 条、250 C 13 条，当前 0 条 ready、25 条 blocked，方法频次包括 `process_feasibility_review=25`、`model_ensemble_recheck=25`、`high_fidelity_simulation_or_expanded_model_ensemble=25`、`thermal_stability_pre_screen=13`。它的证据等级是 `high_fidelity_protocol_template_not_observation`，只作为执行协议，不作为 Tg 证据。
- `scripts/build_validation_dependency_graph.py` 已把上述 request、approval、protocol、result intake 和 active evidence gate 形式化成 DAG：118 个节点、125 条边，当前全部 blocked/pending；下一步动作是审核 12 行 `process_completion_approval_template`。
...
- Workflow summary 已读取 `human_review_target_counts`、`human_review_candidate_origin_counts`、`human_validation_*`、`validation_request_*`、`validation_execution_*`、`process_completion_packet_*`、`process_design_suggestion_*`、`process_approval_*`、`high_fidelity_protocol_*`、`validation_dependency_*`、`validation_result_*`、`active_observation_*`、`active_evidence_pievo_bridge_*`、`todo_completion_*`、全局 strategy policy 和 target-conditioned policy 的 high-authority status；这一步把“真实实验结果迭代优化”前的人工质量门禁、执行排程、工艺补全模板、工艺建议、人工审批入口、高保真协议、约束 DAG、posterior 消费路径、TODO audit 和策略层 no-op 状态落成 artifact，而不是直接把 surrogate 结果冒充真实实验。
