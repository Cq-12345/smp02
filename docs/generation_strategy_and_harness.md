...
8. 250 C sparse target source-pool expansion 已跑通，并已进入 `pre_experiment_validation_plan`、`validation_request_queue` 和 `validation_result_intake_template`：13 条 250 C 候选产生 26 个 request，其中 13 个补工艺、13 个高保真验证。下一步是真实执行这些 request；只有填写结果、工艺 ready、人工批准后，才会进入 observation ledger。
