...
8. 250 C sparse target source-pool expansion 已跑通，并已进入 `pre_experiment_validation_plan`、`validation_request_queue`、`validation_execution_schedule`、`process_completion_packet`、`process_design_suggestion_packet`、`process_approval_intake`、`high_fidelity_protocol_packet`、`validation_result_intake_template`、active evidence gate 和 PiEvo bridge：13 条 250 C 候选产生 26 个 request，其中 13 个补工艺、13 个高保真验证。当前 immediate execution batch 中 250 C 占 8/12，且已展开成可填写 process completion packet；知识模板又进一步给 12 行生成工艺建议，12 行字段可补全。人工审批入口已生成 12 行模板，但当前没有提交审批，因此 `accepted_process_approval_rows=0`、`unblocked_observation_request_rows=0`。High-fidelity protocol packet 已生成 25 条协议，其中 195 C 12 条、250 C 13 条，当前 0 条 ready、25 条仍等待 process approval；它只是方法协议，不是 Tg observation。只有填写结果、工艺 ready、人工批准、observation ledger pass，且来源为高保真/真实/文献，才会进入 active high-authority evidence ledger 并通过 bridge 更新 PiEvo posterior。
