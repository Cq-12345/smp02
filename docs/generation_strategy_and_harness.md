...
5. 生成候选排序时同步读取 `reports/predictor_model_selection_registry.md`、`reports/predictor_ensemble_disagreement.md` 和 `artifacts/trail/predictors/ensemble_disagreement/low_disagreement_near_target.csv`；默认点预测/不确定性代理按 registry 使用 `VAE(512)+GaussianProcess_RBF`，高分歧近目标候选应优先被标记为复核对象，而不是直接推荐。
...
8. `scripts/build_external_generator_output_checklist.py` 已把真实外部 LLM/SFT/decoder/flow 输出接入前的门禁结构化：`llm_rag_principle_generation`、`sft_candidate_generator`、`diffusion_or_flow_matching` 三类可以提交外部 generation records，`llm_smiles_generation` 仍 suppressed；这些输出不直接推荐配方，也不产生 observation。
9. 250 C sparse target source-pool expansion 已跑通，并已进入 `pre_experiment_validation_plan`、`validation_request_queue`、`validation_execution_schedule`、`process_completion_packet`、`process_design_suggestion_packet`、`process_approval_intake`、`process_approval_reviewer_checklist`、`high_fidelity_protocol_packet`、`validation_dependency_graph`、`validation_result_intake_template`、active evidence gate 和 PiEvo bridge：13 条 250 C 候选产生 26 个 request，其中 13 个补工艺、13 个高保真验证。当前 immediate execution batch 中 250 C 占 8/12，且已展开成可填写 process completion packet；知识模板又进一步给 12 行生成工艺建议，12 行字段可补全。人工审批入口已生成 12 行模板，reviewer checklist 显示 12 行均 ready for human review，可解锁 13 条 downstream high-fidelity protocol；但当前没有提交审批，因此 `accepted_process_approval_rows=0`、`unblocked_observation_request_rows=0`。High-fidelity protocol packet 已生成 25 条协议，其中 195 C 12 条、250 C 13 条，当前 0 条 ready、25 条仍等待 process approval；validation dependency graph 有 118 个节点、125 条边，critical path 下一步是审核 12 行 process approval。它们只是协议和约束图，不是 Tg observation。只有填写结果、工艺 ready、人工批准、observation ledger pass，且来源为高保真/真实/文献，才会进入 active high-authority evidence ledger 并通过 bridge 更新 PiEvo posterior。
10. 下一轮生成预算可以读取 `generation_strategy_bandit_policy.csv` 和 `target_conditioned_generation_strategy_policy.csv`，但当前二者分别是 `surrogate_backed_allocation` 与 `target_surrogate_backed_allocation`；每个被分配预算的策略仍必须写入 ledger，并走 predictor/Harness/PiEvo/人工审核链路。
